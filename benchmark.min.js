...
`;HTMLElement.prototype.insert=function(r,t,...e){if(typeof e[0]=="object"){const c=e.shift();t=new Function(Object.keys(c).join(","),"return `"+t+"`")(...Object.values(c))}const n=document.createElement("div");n.innerHTML=t;const o=n.children[0];return this.insertAdjacentElement(r,n.children[0]),[o,...e.map(c=>o.querySelector(c))]},HTMLElement.prototype.append=function(){return this.insert("beforeend",...arguments)},HTMLElement.prototype.prepend=function(){return this.insert("afterbegin",...arguments)},Promise.timeout=function(r){return new Promise(t=>setTimeout(t,r))},Array.prototype.trim=function(r=t=>!t){const t=(o,c)=>!r(o,c,this),e=this.findIndex(t);if(e===this.length-1)return[];const n=this.findLastIndex(t);return this.splice(n+1),this.splice(0,e),this};const containerTpl=`<div class="benchmarks">
...
`).slice(0,-1).join(`
`);scripts=scripts.map((r,t,e)=>{const n=r.split(`
`),o=n.shift().trim();return t!==e.length-1&&n.pop(),{title:o,source:n.join(`
`)}});const max=Math.max(...scripts.map(r=>r.title.length));scripts.forEach(r=>r.title=r.title.padEnd(max," "));const[$container,$benchmarks]=document.body.append(eval("`"+containerTpl+"`"),".test-list"),tests=[];for(const r of scripts){const[t,e]=$benchmarks.append(benchmarkTpl,{formatCode,...r},".eval");(async()=>{let n;try{const[s]=await evalScript(r.source,beforeScript+";");n=JSON.stringify(s,null,4)}catch(s){n=s.message,e.classList.add("error")}let[o,c]=r.source.split(/\/\/\s*@run/);c||([c,o]=[o,c]),o??(o=""),e.innerHTML=[r.title.replace(/\s/g,"&nbsp;")+"&nbsp:",n.toString().slice(0,500)].join("&nbsp;&nbsp;"),tests.push({...r,code:c,pre:o,benchmark:t})})()}$container.querySelector(".cycles").value=scriptContainer.getAttribute("benchmark")||scriptContainer.dataset.count||1,$container.querySelector(".btn-copy").addEventListener("click",r=>{let t=`Cycles: ${document.querySelector("input.cycles").valueAsNumber}
        ---
        ${[...$container.querySelectorAll(".test-list .log")].map(n=>n.textContent).join(`
`)}
        ---
        https://github.com/silentmantra/benchmark`.replace(/^\s+/mg,"").replace("&nbsp;"," ");const e=Math.max(...t.split(`
`).map(n=>n.length));t=t.replaceAll("---","-".repeat(e)),navigator.clipboard.writeText(t)}),$container.querySelector(".run-benchmark").addEventListener("click",async r=>{document.body.classList.add("benchmarking"),r.target.disabled=!0;const t=$container.querySelector("input.cycles").valueAsNumber;for(const e of tests){let n;t===1?[,n]=await benchmarkScript(e.code,beforeScript+";"+e.pre):[,n]=await benchmarkScript(`
                for (let $__i = 0; $__i < ${t}; $__i++) {
                    ${e.code}
                }
            `,beforeScript+";"+e.pre),(e.executions??(e.executions=[])).push(n);const o=e.executions.reduce((s,a)=>s+a);e.average=Math.round(o/e.executions.length),e.min=e.executions.reduce((s,a)=>s===null?a:Math.min(s,a),null);const c=[];e.executions.length>1&&c.push('<b style="color: orange">'+e.min+'</b><span class="ms"></span>(min)'),e.benchmark.querySelector(".log").innerHTML=['<span class="title">'+e.title.replace(/\s/g,"&nbsp;")+"&nbsp:</span>",...c,...e.executions.map(s=>"<b>"+s+"</b>"+(e.executions.length>1?"":'<span class="ms"></span>'))].join("&nbsp;&nbsp;"),e!==tests.at(-1)&&await Promise.timeout(1e3)}tests.sort((e,n)=>e.min-n.min).forEach(e=>$benchmarks.appendChild(e.benchmark)),r.target.disabled=!1})}async function benchmarkScript(r,t=""){const e=document.createElement("script"),n=Math.random().toString().slice(2).split("").reverse().join("");return e.src="data:text/javascript;base64,"+btoa(`
        try {
            ${t}
    ...
    `),document.body.appendChild(e),new Promise((c,s)=>{e.onload=()=>{if(window["__error"+n]){const i=window["__error"+n];delete window["__error"+n],s(i)}const a=window["__result"+n];delete window["__result"+n],c([a])},e.onerror=s})}function formatCode(r){const t=r.split(`
`),e=t.reduce((o,c)=>{const s=c.trim()?c.search(/\S/):1e5;return o===null||s<o?s:o},null);return t.trim().map(o=>o.slice(e)).join(`
`).replace("<","&lt;").replace(">","&gt;")}})();
