...
`;HTMLElement.prototype.insert=function(e,t,...n){if(typeof n[0]=="object"){const i=n.shift();t=new Function(Object.keys(i).join(","),"return `"+t+"`")(...Object.values(i))}const r=document.createElement("div");r.innerHTML=t;const d=r.children[0];return this.insertAdjacentElement(e,r.children[0]),[d,...n.map(i=>d.querySelector(i))]},HTMLElement.prototype.append=function(){return this.insert("beforeend",...arguments)},HTMLElement.prototype.prepend=function(){return this.insert("afterbegin",...arguments)},Promise.timeout=function(e){return new Promise(t=>setTimeout(t,e))},Promise.requestAnimationFrame=function(){return new Promise(e=>requestAnimationFrame(e))},Array.prototype.trim=function(e=t=>!t){const t=(d,i)=>!e(d,i,this),n=this.findIndex(t);if(n===this.length-1)return[];const r=this.findLastIndex(t);return this.splice(r+1),this.splice(0,n),this},Object.defineProperty(Array.prototype,"tail",{get(){return this.at(-1)},set(e){this.push(e)}});class Queue{constructor({depth:t=1,delay:n=0,running:r=()=>!0}={}){v(this,"fns",[]);v(this,"current",[]);this.depth=t,this.delay=n,this.running=r}push(t){return new Promise(n=>{typeof t=="function"?this.fns.push([t,n]):this.fns.forEach(r=>this.push(r)),this.runNext()})}runNext(){if(!this.running()){this.fns.forEach(([n,r])=>r());return}let t=this.depth-this.current.length;for(;t--&&this.fns.length;)(([n,r])=>{this.current.push(n),n().then(async d=>{this.current.splice(this.current.indexOf(n),1),(this.current.length||this.fns.length)&&this.delay&&await Promise.timeout(this.delay),this.runNext(),r(d)})})(this.fns.shift())}stop(){this.running=()=>!1,this.fns.length=0}}const containerTpl=`<div class="benchmarks">
    ...
    <div class="benchmarks"></div>
        ...
        if (Array.isArray($input)){
            ...
            const arr = [];
            while($chunkCount--) arr.push($chunk);
            $input = arr.join('');
        }
...
`).map(h=>h.length));r=r.replaceAll("---","-".repeat(d));const i=document.querySelector("dialog"),b=i.querySelector("textarea");i.style.opacity=0,b.textContent=r,i.showModal(),b.focus(),b.select(),document.execCommand("copy"),i.close()});let running=!1;$container.querySelector(".stop-benchmark").addEventListener("click",()=>running=!1),$container.querySelector(".run-benchmark").addEventListener("click",async e=>{document.body.classList.add("benchmarking","running"),await Promise.timeout(),running=!0,await runBenchmark(),running=!1,document.body.classList.remove("running"),document.querySelector(".run-status").innerHTML=""});async function runBenchmark(){const e=[];groups.forEach(h=>{h.tests.forEach(m=>{m.$benchmark.querySelector(".log").textContent="",m.runs=Array.from(chunks,(o,s)=>e.tail={test:m,chunkCount:o,executions:[],diminisher:1,get duration(){return this.executions[0]??this._estimatedDuration},set duration(a){this._estimatedDuration=a},get count(){return h.count??h.runs[s].count/this.diminisher??1},get min(){return(this.executions[0]??1/0)*this.diminisher}})})}),await Promise.requestAnimationFrame();let t=config("CYCLES",0);const n=+$container.querySelector("input.run-count").valueAsNumber||5,r=$container.querySelector(".run-multicore").checked,d=h=>document.querySelector(".run-status").innerHTML=h+'&nbsp;<span class="blink">...</span>',i={},b=new Queue({depth:numberOfWorkers});t?groups.forEach(h=>h.count=t):(d("estimating cycles"),await Promise.all(groups.map(h=>Promise.all([...chunks.entries()].map(([m,o])=>b.push(async()=>{var u;let s=1,a=h.tests;const l=[];for(;;){for(const c of a)if(c.runs[m].duration=await c.benchmark(s,o,i),!running)return;const p=a.filter(c=>c.runs[m].duration<20);if(l.push(...a.filter(c=>c.runs[m].duration>=100)),l.forEach(c=>c.runs[m].diminisher*=10),!p.length){((u=h.runs??(h.runs=[]))[m]??(u[m]={})).count=s*10;break}a=a.filter(c=>c.runs[m].duration<100),s*=10}}))))));const f=new Queue({depth:r?numberOfWorkers:1,delay:r?0:500,running:()=>running});return Promise.all(Array.from({length:n},(h,m)=>Promise.all([...chunks.entries()].map(([o,s])=>Promise.all(groups.map(({tests:a,$benchmarks:l,runs:u})=>Promise.all(a.map(async p=>f.push(async()=>{var k;const c=p.runs[o];d(formatOrdinals(m+1)+" "+(chunks.length>1?`running ${p.title} with n=${s*getChunkLength()}`:`running ${p.title}`)),(k=p.$benchmark.querySelector(`.run-${o}`))==null||k.classList.add("blink"),await Promise.requestAnimationFrame();const g=await p[r?"benchmarkInWorker":"benchmark"](c.count,s,i);running&&(c.executions.push(g),c.executions.sort((y,$)=>y-$),displayResults())})))))))))}function displayResults(){[...chunks.entries()].forEach(([e,t])=>groups.forEach(({tests:n,$benchmarks:r,compareToTest:d})=>n.forEach(i=>{if(!n.some(o=>{var s;return(s=o.runs)==null?void 0:s.length}))return;n.sort((o,s)=>o.runs[e].min-s.runs[e].min).forEach(o=>r.appendChild(o.$benchmark));const b=(d==null?void 0:d.runs[e].min)??n[0].runs[e].min;n.forEach((o,s)=>{const a=o.runs[e].min/b;if(a===1){o.runs[e].diff="■ 1.00x";return}o.runs[e].diff=isNaN(a)?"":a.toFixed(2)+"x"}),n.sort((o,s)=>(o=o.runs.findLast(a=>a.executions.length),s=s.runs.findLast(a=>a.executions.length),!o||!s?0:o.min-s.min)).forEach(o=>r.appendChild(o.$benchmark));const f=o=>{if(isNaN(o))return"";const s=o.toString().length,a=(s-1)/3|0,l=["","k","m","b"];return"x"+o.toString().slice(0,1+(s-1)%3)+l[a]},h=o=>(o/1e3).toFixed(1).replace(/^0/,"")+"s",m=o=>o.trim()==="■ 1.00x"?"color-green":o.trim().startsWith("0.")?"color-blue":"color-orange";for(const[o,s]of n.entries()){const a=l=>{try{const u=n.map(l);return(u[o]??"").toString().padStart(Math.max(...u.map(c=>(c==null?void 0:c.toString().length)||0))," ")}catch{return""}};if(chunks.length===1){if(!s.runs[e].executions.length)return"";const l=a(u=>u.runs[0].diff);s.$benchmark.querySelector(".log").innerHTML=['<span class="title">'+s.title.replace(/\s/g," ")+" </span>",`<b class="${m(l)} run-0" title="Difference from fastest solution (with fastest results)">${l}</b>`,"|",'<span title="Number of cycles">'+a(u=>"x"+u.runs[0].count)+"</span>",...s.runs[0].executions.map((u,p)=>"<b>"+a(c=>{var g;return(g=c.runs[e].executions)==null?void 0:g[p]})+"</b>")].join(" ")}else{s.$benchmark.querySelector(".log").innerHTML=['<span class="title">'+s.title.replace(/\s/g," ")+" </span>",s.runs.filter(u=>u.executions.length).map((u,p)=>{const c=`n=${chunks[p]*getChunkLength()}`,g=a(y=>y.runs[p].diff);return(`<b class="${m(g)} run-${p}" title="Difference from fastest solution (with fastest results)">`+g+' <span class="color-gray">'+a(y=>f(y.runs[p].count))+" <b>"+a(y=>y.runs[p].duration)+"</b></span></b>").padStart(c.length," ")}).join(" | ")].join("  ");const l=document.querySelector(".run-title");l.innerHTML=[" ".repeat(s.title.length)+" ",chunks.map((u,p)=>{const c=`n=${u*getChunkLength()}`;if(!s.runs[p].executions)return"";const g=a(k=>k.runs[p].diff)+" "+a(k=>f(k.runs[p].count))+" "+a(k=>k.runs[p].duration);return c.padStart((g.length+c.length)/2," ").padEnd(g.length," ")}).filter(Boolean).join(" | ")].join("  ")}}})))}}function executeFunctionInWorker(e){return new Promise(t=>{const n=new Blob([`
