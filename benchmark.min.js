...
`;HTMLElement.prototype.insert=function(e,t,...n){if(typeof n[0]=="object"){const i=n.shift();t=new Function(Object.keys(i).join(","),"return `"+t+"`")(...Object.values(i))}const r=document.createElement("div");r.innerHTML=t;const m=r.children[0];return this.insertAdjacentElement(e,r.children[0]),[m,...n.map(i=>m.querySelector(i))]},HTMLElement.prototype.append=function(){return this.insert("beforeend",...arguments)},HTMLElement.prototype.prepend=function(){return this.insert("afterbegin",...arguments)},Promise.timeout=function(e){return new Promise(t=>setTimeout(t,e))},Promise.requestAnimationFrame=function(){return new Promise(e=>requestAnimationFrame(e))},Array.prototype.trim=function(e=t=>!t){const t=(m,i)=>!e(m,i,this),n=this.findIndex(t);if(n===this.length-1)return[];const r=this.findLastIndex(t);return this.splice(r+1),this.splice(0,n),this},Object.defineProperty(Array.prototype,"tail",{get(){return this.at(-1)},set(e){this.push(e)}});class Queue{constructor({depth:t=1,delay:n=0,running:r=()=>!0}={}){w(this,"fns",[]);w(this,"current",[]);this.depth=t,this.delay=n,this.running=r}push(t){return new Promise(n=>{typeof t=="function"?this.fns.push([t,n]):this.fns.forEach(r=>this.push(r)),this.runNext()})}runNext(){if(!this.running()){this.fns.forEach(([n,r])=>r());return}let t=this.depth-this.current.length;for(;t--&&this.fns.length;)(([n,r])=>{this.current.push(n),n().then(async m=>{this.current.splice(this.current.indexOf(n),1),(this.current.length||this.fns.length)&&this.delay&&await Promise.timeout(this.delay),this.runNext(),r(m)})})(this.fns.shift())}stop(){this.running=()=>!1,this.fns.length=0}}const containerTpl=`<div class="benchmarks">
...
`}script&&scripts.push(script);{const e=Math.max(...scripts.map(t=>t.title.length));scripts.forEach(t=>t.title=t.title.padEnd(e," "))}let numberOfWorkers=Math.max(1,Math.min(6,navigator.hardwareConcurrency/2|0));const multicore=config("WORKERS");typeof multicore=="number"&&(numberOfWorkers=multicore);const[$container,$groups]=document.body.append(eval("`"+containerTpl+"`"),".test-list");$container.querySelector(".run-count").value=config("TIMES",5),$container.querySelector(".run-multicore").checked=!!multicore,$container.querySelector(".btn-multicore-help").addEventListener("click",e=>{e.preventDefault(),alert("Use workers for development and getting fast estimation results. They are less precise (more difference between test cases). For publishing precise results use benchmarking on the main thread.")}),(async()=>{const e=$container.querySelector(".memory-heap");for(;;)await Promise.requestAnimationFrame(),e.textContent=Math.round(performance.memory.usedJSHeapSize/1024/1024)+"Mb"})();const groups=[];for(const e of Object.keys(_groups)){const[t,n]=$groups.append(groupTpl,{title:e},".benchmarks"),r={title:e,$group:t,$benchmarks:n,tests:[]};n.addEventListener("click",i=>{const b=i.target.closest(".log");if(!b)return;const f=b.dataset.idx;debugger;r.compareToTest=r.tests.find(p=>p.idx===+f),displayResults()}),document.addEventListener("mousedown",i=>{if(!n.contains(i.target)){debugger;delete r.compareToTest,displayResults()}});let m=-1;for(const i of scripts.filter(({group:b})=>b===e))m++,(async b=>{const[f,p]=n.append(benchmarkTpl,{...i,idx:b},".eval");let d;try{const[l]=await evalScript(i.source,getBeforeScript());d=JSON.stringify(l,null,4)}catch(l){d=l.message,p.classList.add("error")}i.source=i.source.trim(),i.source[0]==="{"&&i.source.at(-1)==="}"&&(i.source=i.source.slice(1,-1));let[o,s]=i.source.split(/\/\/\s*@run/);s||([s,o]=[o,s]),o??(o=""),p.innerHTML=[i.title.replace(/\s/g,"&nbsp;")+"&nbsp:",d===void 0?"undefined":d.toString().slice(0,500)].join("&nbsp;&nbsp;");let a;r.tests.push(a={...i,idx:b,code:s,pre:o,$benchmark:f,async benchmarkClone(l,h,u){var k,y;const c=+$container.querySelector("input.run-count").valueAsNumber||5,g=(y=(k=u.clone??(u.clone={}))[h]??(k[h]={}))[l]??(y[l]={duration:1/0,benchmarkedCount:0});if(g.benchmarkedCount<c){g.benchmarkedCount++;const v=await benchmarkScript(`for (let $__i = 0; $__i < ${l}; $__i++) {$clone()}`,getBeforeScript(h,o));v<g.duration&&(g.duration=v)}return g.duration},async benchmark(l=1,h=1,u={}){if(l===1)return benchmarkScript(s,getBeforeScript(h,o));if(config("$clone")){const g=await this.benchmarkClone(l,h,u);return await benchmarkScript(`for (let $__i = 0; $__i < ${l}; $__i++) {$input = $clone();${s}}`,getBeforeScript(h,o))-g}return benchmarkScript(`for (let $__i = 0; $__i < ${l}; $__i++) {${s}}`,getBeforeScript(h,o))},async benchmarkInWorker(l=1,h=1){let u=getBeforeScript(h,o),c;l===1?c=s:c=`for (let $__i = 0; $__i < ${l}; $__i++) {${s}}`;const g=new Function("",`
                            const result = {};
                            try {
                                ${u}
                            ...
                            return result;
                        `);return(await executeFunctionInWorker(g)).duration}})})(m);groups.push(r)}$container.querySelector(".btn-copy").addEventListener("click",e=>{var f;const t=(f=navigator.userAgent.match(/(Chrome|Firefox)\/\d+/))==null?void 0:f[0];let n=document.querySelector(".run-title").textContent;n.length&&(n=n.replace(/^./,">"),n+=`
`);let r=`
        \`\`\`
        \` ${t}
        ---
        ${n}${groups.map(p=>{const d=p.title==="_default",o=[];return d||o.push(p.title),p.$benchmarks.querySelectorAll(".log").forEach(s=>{o.push((d?"":"&nbsp;&nbsp;&nbsp;&nbsp;")+s.textContent)}),o.join(`
`)}).join(`
`)}
        ---
        https://github.com/silentmantra/benchmark \`
        \`\`\``.replace(/^\s+/mg,"").replaceAll("&nbsp;"," ");const m=Math.max(...r.split(`
`).map(p=>p.length));r=r.replaceAll("---","-".repeat(m));const i=document.querySelector("dialog"),b=i.querySelector("textarea");i.style.opacity=0,b.textContent=r,i.showModal(),b.focus(),b.select(),document.execCommand("copy"),i.close()});let running=!1;$container.querySelector(".stop-benchmark").addEventListener("click",()=>running=!1),$container.querySelector(".run-benchmark").addEventListener("click",async e=>{document.body.classList.add("benchmarking","running"),await Promise.timeout(),running=!0,await runBenchmark(),running=!1,document.body.classList.remove("running"),document.querySelector(".run-status").innerHTML=""});async function runBenchmark(){const e=[];groups.forEach(p=>{p.tests.forEach(d=>{d.$benchmark.querySelector(".log").textContent="",d.runs=Array.from(chunks,(o,s)=>e.tail={test:d,chunkCount:o,executions:[],diminisher:1,get duration(){return this.executions[0]??this._estimatedDuration},set duration(a){this._estimatedDuration=a},get count(){return p.count??p.runs[s].count/this.diminisher??1},get min(){return(this.executions[0]??1/0)*this.diminisher}})})}),await Promise.requestAnimationFrame();let t=config("CYCLES",0);const n=+$container.querySelector("input.run-count").valueAsNumber||5,r=$container.querySelector(".run-multicore").checked,m=p=>document.querySelector(".run-status").innerHTML=p+'&nbsp;<span class="blink">...</span>',i={},b=new Queue({depth:numberOfWorkers});t?groups.forEach(p=>p.count=t):(m("estimating cycles"),await Promise.all(groups.map(p=>Promise.all([...chunks.entries()].map(([d,o])=>b.push(async()=>{var h;let s=1,a=p.tests;const l=[];for(;;){for(const c of a)if(c.runs[d].duration=await c.benchmark(s,o,i),!running)return;const u=a.filter(c=>c.runs[d].duration<20);if(l.push(...a.filter(c=>c.runs[d].duration>=100)),l.forEach(c=>c.runs[d].diminisher*=10),!u.length){((h=p.runs??(p.runs=[]))[d]??(h[d]={})).count=s*10;break}a=a.filter(c=>c.runs[d].duration<100),s*=10}}))))));const f=new Queue({depth:r?numberOfWorkers:1,delay:r?0:500,running:()=>running});return Promise.all(Array.from({length:n},(p,d)=>Promise.all([...chunks.entries()].map(([o,s])=>Promise.all(groups.map(({tests:a,$benchmarks:l,runs:h})=>Promise.all(a.map(async u=>f.push(async()=>{var k;const c=u.runs[o];m(formatOrdinals(d+1)+" "+(chunks.length>1?`running ${u.title} with n=${s*getChunkLength()}`:`running ${u.title}`)),(k=u.$benchmark.querySelector(`.run-${o}`))==null||k.classList.add("blink"),await Promise.requestAnimationFrame();const g=await u[r?"benchmarkInWorker":"benchmark"](c.count,s,i);running&&(c.executions.push(g),c.executions.sort((y,v)=>y-v),displayResults())})))))))))}function displayResults(){[...chunks.entries()].forEach(([e,t])=>groups.forEach(({tests:n,$benchmarks:r,compareToTest:m})=>n.forEach(i=>{n.sort((o,s)=>o.runs[e].min-s.runs[e].min).forEach(o=>r.appendChild(o.$benchmark));const b=(m==null?void 0:m.runs[e].min)??n[0].runs[e].min;n.forEach((o,s)=>{const a=o.runs[e].min/b;if(a===1){o.runs[e].diff="■ 1.00x";return}o.runs[e].diff=isNaN(a)?"":a.toFixed(2)+"x"}),n.sort((o,s)=>(o=o.runs.findLast(a=>a.executions.length),s=s.runs.findLast(a=>a.executions.length),!o||!s?0:o.min-s.min)).forEach(o=>r.appendChild(o.$benchmark));const f=o=>{if(isNaN(o))return"";const s=o.toString().length,a=(s-1)/3|0,l=["","k","m","b"];return"x"+o.toString().slice(0,1+(s-1)%3)+l[a]},p=o=>(o/1e3).toFixed(1).replace(/^0/,"")+"s",d=o=>o.trim()==="■ 1.00x"?"color-green":o.trim().startsWith("0.")?"color-blue":"color-orange";for(const[o,s]of n.entries()){const a=l=>{const h=n.map(l);return(h[o]??"").toString().padStart(Math.max(...h.map(c=>(c==null?void 0:c.toString().length)||0))," ")};if(chunks.length===1)s.$benchmark.querySelector(".log").innerHTML=['<span class="title">'+s.title.replace(/\s/g," ")+" </span>",'<b class="color-orange run-0" title="Difference from fastest solution (with fastest results)">'+a(l=>l.runs[0].diff)+"</b>","|",'<span title="Number of cycles">'+a(l=>"x"+l.runs[0].count)+"</span>",...s.runs[0].executions.map((l,h)=>"<b>"+a(u=>{var c;return(c=u.runs[e].executions)==null?void 0:c[h]})+"</b>")].join(" ");else{s.$benchmark.querySelector(".log").innerHTML=['<span class="title">'+s.title.replace(/\s/g," ")+" </span>",s.runs.filter(h=>h.executions.length).map((h,u)=>{const c=`n=${chunks[u]*getChunkLength()}`,g=a(y=>y.runs[u].diff);return(`<b class="${d(g)} run-${u}" title="Difference from fastest solution (with fastest results)">`+g+' <span class="color-gray">'+a(y=>f(y.runs[u].count))+" <b>"+a(y=>y.runs[u].duration)+"</b></span></b>").padStart(c.length," ")}).join(" | ")].join("  ");const l=document.querySelector(".run-title");l.innerHTML=[" ".repeat(s.title.length)+" ",chunks.map((h,u)=>{const c=`n=${h*getChunkLength()}`;if(!s.runs[u].executions)return"";const g=a(k=>k.runs[u].diff)+" "+a(k=>f(k.runs[u].count))+" "+a(k=>k.runs[u].duration);return c.padStart((g.length+c.length)/2," ").padEnd(g.length," ")}).filter(Boolean).join(" | ")].join("  ")}}})))}}function executeFunctionInWorker(e){return new Promise(t=>{const n=new Blob([`
      let start = performance.now();
      const result = (${e.toString()})();
      postMessage({result, duration: performance.now() - start});
      `],{type:"application/javascript"});new Worker(URL.createObjectURL(n)).addEventListener("message",m=>{t(m.data)})})}async function benchmarkScript(e,t=""){const n=document.createElement("script"),r=Math.random().toString().slice(2).split("").reverse().join("");return n.src="data:text/javascript;base64,"+btoa(`
        try {
            ${t}
            const __$start$ = performance.now();
            ${e};
    ...
    `),document.body.appendChild(n),new Promise((m,i)=>{n.onload=()=>{if(window["__error"+r]){const f=window["__error"+r];delete window["__error"+r],i(f)}const b=window["__duration"+r];delete window["__duration"+r],location.host==="localhost"||n.remove(),m(b)},n.onerror=i})}async function evalScript(e,t=""){const n=document.createElement("script"),r=Math.random().toString().slice(2).split("").reverse().join(""),m=e.replaceAll("\\","\\\\").replaceAll("`","\\`").replaceAll("${","\\${");return n.src="data:text/javascript;base64,"+btoa(`
        try {
            ${t}
            window.__result${r} = eval(\`${m}\`);
        } catch (e) {
            window.__error${r} = e;
        }
    `),document.body.appendChild(n),new Promise((i,b)=>{n.onload=()=>{if(window["__error"+r]){const p=window["__error"+r];delete window["__error"+r],b(p)}const f=window["__result"+r];delete window["__result"+r],location.host==="localhost"||n.remove(),i([f])},n.onerror=b})}function config(name,...vals){try{return eval(name)}catch(e){return vals.find(Boolean)}}})();
