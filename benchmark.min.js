...
`;HTMLElement.prototype.insert=function(e,i,...r){if(typeof r[0]=="object"){const t=r.shift();i=new Function(Object.keys(t).join(","),"return `"+i+"`")(...Object.values(t))}const o=document.createElement("div");o.innerHTML=i;const p=o.children[0];return this.insertAdjacentElement(e,o.children[0]),[p,...r.map(t=>p.querySelector(t))]},HTMLElement.prototype.append=function(){return this.insert("beforeend",...arguments)},HTMLElement.prototype.prepend=function(){return this.insert("afterbegin",...arguments)},Promise.timeout=function(e){return new Promise(i=>setTimeout(i,e))},Promise.requestAnimationFrame=function(){return new Promise(e=>requestAnimationFrame(e))},Array.prototype.trim=function(e=i=>!i){const i=(p,t)=>!e(p,t,this),r=this.findIndex(i);if(r===this.length-1)return[];const o=this.findLastIndex(i);return this.splice(o+1),this.splice(0,r),this};const containerTpl=`<div class="benchmarks">
    ...
    `;let beforeScript="";const scripts=[];let group="_default";const _groups={};let script,commented=!1;for(let e of scriptContainer.textContent.split(`
`)){if(/^\s*\/\*/.test(e)){commented=!0;continue}else if(commented&&/\*\/\s*$/.test(e)){commented=!1;continue}else if(commented)continue;if(/^\s*\/\/\s*@(end|footer)|^\s*\/\*\s*@end\s*\*\//.test(e))break;const i=e.match(new RegExp("(?<=@group\\s+).+(?=\\s*)$"));if(i){group=i[0];continue}const r=e.match(new RegExp("(?<=@benchmark\\s+).+(?=\\s*)$"));if(r){script&&scripts.push(script),_groups[group]=!0,script={title:r[0],group,source:""};continue}if(script){script.source+=e+`
`;continue}e.match(/^\s*const\s+\$input/)&&(e+="/* @input fill */"),beforeScript+=e+`
`}script&&scripts.push(script);{const e=Math.max(...scripts.map(i=>i.title.length));scripts.forEach(i=>i.title=i.title.padEnd(e," "))}const[$container,$groups]=document.body.append(eval("`"+containerTpl+"`"),".test-list"),groups=[];for(const e of Object.keys(_groups)){const[i,r]=$groups.append(groupTpl,{title:e},".benchmarks"),o={title:e,$group:i,$benchmarks:r,tests:[]};for(const p of scripts.filter(({group:t})=>t===e)){const[t,n]=r.append(benchmarkTpl,{...p},".eval");(async()=>{let d;try{const h=config("$chunk");let y=beforeScript;h&&(y=y.replace("/* @input fill */",inputFillScript()));const[l]=await evalScript(p.source,y+";");d=JSON.stringify(l,null,4)}catch(h){d=h.message,n.classList.add("error")}let[s,m]=p.source.split(/\/\/\s*@run/);m||([m,s]=[s,m]),s??(s=""),n.innerHTML=[p.title.replace(/\s/g,"&nbsp;")+"&nbsp:",d.toString().slice(0,500)].join("&nbsp;&nbsp;"),o.tests.push({...p,code:m,pre:s,$benchmark:t,benchmark(h=1,y=1){const l=config("$chunk");let g=beforeScript;return l&&(g=g.replace("/* @input fill */",inputFillScript(y))),g+=";"+s,benchmarkScript(h===1?m:`for (let $__i = 0; $__i < ${h}; $__i++) {${m}}`,g)}})})()}groups.push(o)}$container.querySelector(".cycles").value=config("CYCLES",scriptContainer.getAttribute("benchmark"),scriptContainer.dataset.count),$container.querySelector(".run-count").value=config("TIMES",5),$container.querySelector(".btn-copy").addEventListener("click",e=>{var d;const i=(d=navigator.userAgent.match(/(Chrome|Firefox)\/\d+/))==null?void 0:d[0];let r=document.querySelector(".run-title").textContent;r.length&&(r=r.replace(/^./,">"),r+=`
`);let o=`
        \`\`\`
        \` ${i}
        ---
        ${r}${groups.map(s=>{const m=s.title==="_default",h=[];return m||h.push(s.title),s.$benchmarks.querySelectorAll(".log").forEach(y=>{h.push((m?"":"&nbsp;&nbsp;&nbsp;&nbsp;")+y.textContent)}),h.join(`
`)}).join(`
`)}
        ---
        https://github.com/silentmantra/benchmark \`
        \`\`\``.replace(/^\s+/mg,"").replaceAll("&nbsp;"," ");const p=Math.max(...o.split(`
`).map(s=>s.length));o=o.replaceAll("---","-".repeat(p));const t=document.querySelector("dialog"),n=t.querySelector("textarea");t.style.opacity=0,n.textContent=o,t.showModal(),n.focus(),n.select(),document.execCommand("copy"),t.close()});let running=!1;$container.querySelector(".stop-benchmark").addEventListener("click",()=>running=!1),$container.querySelector(".run-benchmark").addEventListener("click",async e=>{document.body.classList.add("benchmarking","running"),await Promise.timeout(),running=!0,await runBenchmark(),running=!1,document.body.classList.remove("running"),document.querySelector(".run-status").innerHTML=""});async function runBenchmark(){var o,p;groups.forEach(t=>{t.tests.forEach(n=>{n.$benchmark.querySelector(".log").textContent="",n.runs=Array.from(chunks,()=>({diminisher:1}))})}),await Promise.requestAnimationFrame();let e=$container.querySelector("input.cycles").valueAsNumber;const i=+$container.querySelector("input.run-count").valueAsNumber||5,r=t=>document.querySelector(".run-status").innerHTML=t+'&nbsp;<span class="blink">...</span>';if(e)groups.forEach(t=>t.count=e);else{r("estimating cycles");for(const t of groups)for(const[n,d]of chunks.entries()){let s=1,m=t.tests;const h=[];for(;;){for(const l of m)if(l.runs[n].duration=await l.benchmark(s,d),!running)return;const y=m.filter(l=>l.runs[n].duration<20);if(h.push(...m.filter(l=>l.runs[n].duration>=100)),h.forEach(l=>l.runs[n].diminisher*=10),!y.length){((o=t.runs??(t.runs=[]))[n]??(o[n]={})).count=s*10;break}m=m.filter(l=>l.runs[n].duration<100),s*=10}}}for(let t=0;t<i;t++)for(const[n,d]of chunks.entries()){for(const{tests:s,$benchmarks:m,runs:h}of groups)for(const[y,l]of[...s].entries()){const g=l.runs[n];r(chunks.length>1?`running ${l.title} with n=${d*getChunkLength()}`:`running ${l.title}`),(p=l.$benchmark.querySelector(`.run-${n}`))==null||p.classList.add("blink"),await Promise.requestAnimationFrame();const _=await l.benchmark(h[n].count/g.diminisher|0,d);if(!running)return;(g.executions??(g.executions=[])).push(_),g.executions.sort((c,a)=>c-a),g.min=g.executions[0],s.sort((c,a)=>c.runs[n].min*c.runs[n].diminisher-a.runs[n].min*a.runs[n].diminisher).forEach(c=>m.appendChild(c.$benchmark));const C=(s[0].runs[n].min||0)*s[0].runs[n].diminisher;s.forEach((c,a)=>{if(a){const u=c.runs[n].min*c.runs[n].diminisher/C;c.runs[n].diff=isNaN(u)?"":u.toFixed(2)+"x"}else c.runs[n].diff="1.00x"}),s.sort((c,a)=>(c=c.runs.findLast(u=>u.executions),a=a.runs.findLast(u=>u.executions),!c||!a?0:c.min*c.diminisher-a.min*a.diminisher)).forEach(c=>m.appendChild(c.$benchmark));const S=c=>{if(isNaN(c))return"";const a=c.toString().length,u=(a-1)/3|0,v=["","k","m","b"];return"x"+c.toString().slice(0,1+(a-1)%3)+v[u]},L=c=>(c/1e3).toFixed(1).replace(/^0/,"")+"s";for(const[c,a]of s.filter(u=>u.runs[n].executions).entries()){const u=v=>{const w=s.map(v);return(w[c]??"").toString().padStart(Math.max(...w.map(f=>(f==null?void 0:f.toString().length)||0))," ")};if(chunks.length===1)a.$benchmark.querySelector(".log").innerHTML=['<span class="title">'+a.title.replace(/\s/g," ")+" </span>",'<b class="color-orange run-0" title="Difference from fastest solution (with fastest results)">'+u(v=>v.runs[0].diff)+"</b>","|",'<span title="Number of cycles">'+u(v=>"x"+h[0].count/v.runs[0].diminisher)+"</span>",...a.runs[0].executions.map((v,w)=>"<b>"+u(b=>{var f;return(f=b.runs[n].executions)==null?void 0:f[w]})+"</b>")].join(" ");else{a.$benchmark.querySelector(".log").innerHTML=['<span class="title">'+a.title.replace(/\s/g," ")+" </span>",a.runs.filter(w=>w.executions).map((w,b)=>{const f=`n=${chunks[b]*getChunkLength()}`;return(`<b class="color-orange run-${b}" title="Difference from fastest solution (with fastest results)">`+u(k=>k.runs[b].diff)+' <span class="color-gray">'+u(k=>S(h[b].count/k.runs[b].diminisher))+" <b>"+u(k=>{var x;return(x=k.runs[b].executions)==null?void 0:x[0]})+"</b></span></b>").padStart(f.length," ")}).join(" | ")].join("  ");const v=document.querySelector(".run-title");v.innerHTML=[" ".repeat(a.title.length)+" ",chunks.map((w,b)=>{const f=`n=${w*getChunkLength()}`;if(!a.runs[b].executions)return"";const $=u(k=>k.runs[b].diff)+" "+u(k=>S(h[b].count/k.runs[b].diminisher))+" "+u(k=>{var x;return(x=k.runs[b].executions)==null?void 0:x[0]});return f.padStart(($.length+f.length)/2," ").padEnd($.length," ")}).filter(Boolean).join(" | ")].join("  ")}}if(l!==s.at(-1)&&await Promise.timeout(1e3),!running)return}if(t<i.length-1&&await Promise.timeout(1e3),!running)return}}}async function benchmarkScript(e,i=""){const r=document.createElement("script"),o=Math.random().toString().slice(2).split("").reverse().join("");return r.src="data:text/javascript;base64,"+btoa(`
        try {
            ${i}
    ...
    `),document.body.appendChild(r),new Promise((p,t)=>{r.onload=()=>{if(window["__error"+o]){const d=window["__error"+o];delete window["__error"+o],t(d)}const n=window["__duration"+o];delete window["__duration"+o],r.remove(),p(n)},r.onerror=t})}async function evalScript(e,i=""){const r=document.createElement("script"),o=Math.random().toString().slice(2).split("").reverse().join(""),p="`"+e.replaceAll("\\","\\\\").replaceAll("`","\\`")+"`";return r.src="data:text/javascript;base64,"+btoa(`
        try {
            ${i}
    ...
    `),document.body.appendChild(r),new Promise((t,n)=>{r.onload=()=>{if(window["__error"+o]){const s=window["__error"+o];delete window["__error"+o],n(s)}const d=window["__result"+o];delete window["__result"+o],r.remove(),t([d])},r.onerror=n})}function config(name,...vals){try{return eval(name)}catch(e){return vals.find(Boolean)}}})();
