...
`;HTMLElement.prototype.insert=function(e,t,...n){if(typeof n[0]=="object"){const l=n.shift();t=new Function(Object.keys(l).join(","),"return `"+t+"`")(...Object.values(l))}const r=document.createElement("div");r.innerHTML=t;const c=r.children[0];return this.insertAdjacentElement(e,r.children[0]),[c,...n.map(l=>c.querySelector(l))]},HTMLElement.prototype.append=function(){return this.insert("beforeend",...arguments)},HTMLElement.prototype.prepend=function(){return this.insert("afterbegin",...arguments)},Promise.timeout=function(e){return new Promise(t=>setTimeout(t,e))},Promise.requestAnimationFrame=function(){return new Promise(e=>requestAnimationFrame(e))},Array.prototype.trim=function(e=t=>!t){const t=(c,l)=>!e(c,l,this),n=this.findIndex(t);if(n===this.length-1)return[];const r=this.findLastIndex(t);return this.splice(r+1),this.splice(0,n),this},Object.defineProperty(Array.prototype,"tail",{get(){return this.at(-1)},set(e){this.push(e)}});class Queue{constructor({depth:t=1,delay:n=0,running:r=()=>!0}={}){C(this,"fns",[]);C(this,"current",[]);this.depth=t,this.delay=n,this.running=r}push(t){return new Promise(n=>{typeof t=="function"?this.fns.push([t,n]):this.fns.forEach(r=>this.push(r)),this.runNext()})}runNext(){if(!this.running()){this.fns.forEach(([n,r])=>r());return}let t=this.depth-this.current.length;for(;t--&&this.fns.length;)(([n,r])=>{this.current.push(n),n().then(async c=>{this.current.splice(this.current.indexOf(n),1),(this.current.length||this.fns.length)&&this.delay&&await Promise.timeout(this.delay),this.runNext(),r(c)})})(this.fns.shift())}stop(){this.running=()=>!1,this.fns.length=0}}const containerTpl=`<div class="benchmarks">
    ...
    <div class="row toolbar">
        ...
        <label class="checkbox"><input class="run-multicore" type="checkbox" style="position:relative;top:2px">
    ...
    `,lengthMultiplyScript=(e=1)=>`
         $length *= ${e};
    `,getBeforeScript=(e=1,t="")=>{let n=beforeScript;return $chunk&&(n=n.replace("/* @input fill */",inputFillScript(e))),$length&&(n=n.replace("/* @length multiply */",lengthMultiplyScript(e))),t&&(n+=";"+t),n+=";",n};let beforeScript="";const scripts=[];let group="_default";const _groups={};let script,commented=!1;for(let e of scriptContainer.textContent.split(`
...
`}script&&scripts.push(script);{const e=Math.max(...scripts.map(t=>t.title.length));scripts.forEach(t=>t.title=t.title.padEnd(e," "))}let numberOfWorkers=Math.max(1,Math.min(6,navigator.hardwareConcurrency/2|0));const multicore=config("WORKERS");typeof multicore=="number"&&(numberOfWorkers=multicore);const[$container,$groups]=document.body.append(eval("`"+containerTpl+"`"),".test-list");$container.querySelector(".run-count").value=config("TIMES",5),$container.querySelector(".run-multicore").checked=!!multicore,$container.querySelector(".btn-multicore-help").addEventListener("click",e=>{e.preventDefault(),alert("Use workers for development and getting fast estimation results. They are less precise (more difference between test cases). For publishing precise results use benchmarking on the main thread.")}),(async()=>{const e=$container.querySelector(".memory-heap");for(;;)await Promise.requestAnimationFrame(),e.textContent=Math.round(performance.memory.usedJSHeapSize/1024/1024)+"Mb"})();const groups=[];for(const e of Object.keys(_groups)){const[t,n]=$groups.append(groupTpl,{title:e},".benchmarks"),r={title:e,$group:t,$benchmarks:n,tests:[]};for(const c of scripts.filter(({group:l})=>l===e)){const[l,d]=n.append(benchmarkTpl,{...c},".eval");(async()=>{let b;try{const[s]=await evalScript(c.source,getBeforeScript());b=JSON.stringify(s,null,4)}catch(s){b=s.message,d.classList.add("error")}let[o,i]=c.source.split(/\/\/\s*@run/);i||([i,o]=[o,i]),o??(o=""),d.innerHTML=[c.title.replace(/\s/g,"&nbsp;")+"&nbsp:",b.toString().slice(0,500)].join("&nbsp;&nbsp;"),r.tests.push({...c,code:i,pre:o,$benchmark:l,benchmark(s=1,h=1){return benchmarkScript(s===1?i:`for (let $__i = 0; $__i < ${s}; $__i++) {${i}}`,getBeforeScript(h,o))},async benchmarkInWorker(s=1,h=1){let f=getBeforeScript(h,o),x;s===1?x=i:x=`for (let $__i = 0; $__i < ${s}; $__i++) {${i}}`;const _=new Function("",`
                            ...
                            return result;
                        `);return(await executeFunctionInWorker(_)).duration}})})()}groups.push(r)}$container.querySelector(".btn-copy").addEventListener("click",e=>{var b;const t=(b=navigator.userAgent.match(/(Chrome|Firefox)\/\d+/))==null?void 0:b[0];let n=document.querySelector(".run-title").textContent;n.length&&(n=n.replace(/^./,">"),n+=`
`);let r=`
        \`\`\`
        \` ${t}
        ---
        ${n}${groups.map(o=>{const i=o.title==="_default",s=[];return i||s.push(o.title),o.$benchmarks.querySelectorAll(".log").forEach(h=>{s.push((i?"":"&nbsp;&nbsp;&nbsp;&nbsp;")+h.textContent)}),s.join(`
`)}).join(`
`)}
        ---
        https://github.com/silentmantra/benchmark \`
        \`\`\``.replace(/^\s+/mg,"").replaceAll("&nbsp;"," ");const c=Math.max(...r.split(`
`).map(o=>o.length));r=r.replaceAll("---","-".repeat(c));const l=document.querySelector("dialog"),d=l.querySelector("textarea");l.style.opacity=0,d.textContent=r,l.showModal(),d.focus(),d.select(),document.execCommand("copy"),l.close()});let running=!1;$container.querySelector(".stop-benchmark").addEventListener("click",()=>running=!1),$container.querySelector(".run-benchmark").addEventListener("click",async e=>{document.body.classList.add("benchmarking","running"),await Promise.timeout(),running=!0,await runBenchmark(),running=!1,document.body.classList.remove("running"),document.querySelector(".run-status").innerHTML=""});async function runBenchmark(){const e=[];groups.forEach(o=>{o.tests.forEach(i=>{i.$benchmark.querySelector(".log").textContent="",i.runs=Array.from(chunks,(s,h)=>e.tail={test:i,chunkCount:s,executions:[],diminisher:1,get duration(){return this.executions[0]??this._estimatedDuration},set duration(f){this._estimatedDuration=f},get count(){return o.count??o.runs[h].count/this.diminisher??1},get min(){return(this.executions[0]??this.duration)*this.diminisher}})})}),await Promise.requestAnimationFrame();let t=config(CYCLES,0);const n=+$container.querySelector("input.run-count").valueAsNumber||5,r=$container.querySelector(".run-multicore").checked,c=o=>document.querySelector(".run-status").innerHTML=o+'&nbsp;<span class="blink">...</span>',l=new Queue({depth:numberOfWorkers});t?groups.forEach(o=>o.count=t):(c("estimating cycles"),await Promise.all(groups.map(o=>Promise.all([...chunks.entries()].map(([i,s])=>l.push(async()=>{var _;let h=1,f=o.tests;const x=[];for(;;){for(const m of f)if(m.runs[i].duration=await m.benchmark(h,s),!running)return;const y=f.filter(m=>m.runs[i].duration<20);if(x.push(...f.filter(m=>m.runs[i].duration>=100)),x.forEach(m=>m.runs[i].diminisher*=10),!y.length){((_=o.runs??(o.runs=[]))[i]??(_[i]={})).count=h*10;break}f=f.filter(m=>m.runs[i].duration<100),h*=10}})))))),c("estimating results with workers (multi-threading)"),await Promise.all(e.map(o=>l.push(async()=>{o.duration=await o.test.benchmarkInWorker(o.count,o.chunkCount)}))),b();const d=new Queue({depth:r?numberOfWorkers:1,delay:r?0:500,running:()=>running});return Promise.all(Array.from({length:n},(o,i)=>Promise.all([...chunks.entries()].map(([s,h])=>Promise.all(groups.map(({tests:f,$benchmarks:x,runs:_})=>Promise.all(f.map(async y=>d.push(async()=>{var u;const m=y.runs[s];c(formatOrdinals(i+1)+" "+(chunks.length>1?`running ${y.title} with n=${h*getChunkLength()}`:`running ${y.title}`)),(u=y.$benchmark.querySelector(`.run-${s}`))==null||u.classList.add("blink"),await Promise.requestAnimationFrame();const a=await y[r?"benchmarkInWorker":"benchmark"](m.count,h);running&&(m.executions.push(a),m.executions.sort((p,k)=>p-k),b())})))))))));function b(){[...chunks.entries()].forEach(([o,i])=>groups.forEach(({tests:s,$benchmarks:h,runs:f})=>s.forEach(x=>{s.sort((a,u)=>a.runs[o].min-u.runs[o].min).forEach(a=>h.appendChild(a.$benchmark));const _=s[0].runs[o].min;s.forEach((a,u)=>{if(u){const p=a.runs[o].min/_;a.runs[o].diff=isNaN(p)?"":p.toFixed(2)+"x"}else a.runs[o].diff="1.00x"}),s.sort((a,u)=>(a=a.runs.findLast(p=>p.executions.length),u=u.runs.findLast(p=>p.executions.length),!a||!u?0:a.min-u.min)).forEach(a=>h.appendChild(a.$benchmark));const y=a=>{if(isNaN(a))return"";const u=a.toString().length,p=(u-1)/3|0,k=["","k","m","b"];return"x"+a.toString().slice(0,1+(u-1)%3)+k[p]},m=a=>(a/1e3).toFixed(1).replace(/^0/,"")+"s";for(const[a,u]of s.entries()){const p=k=>{const $=s.map(k);return($[a]??"").toString().padStart(Math.max(...$.map(w=>(w==null?void 0:w.toString().length)||0))," ")};if(chunks.length===1)u.$benchmark.querySelector(".log").innerHTML=['<span class="title">'+u.title.replace(/\s/g," ")+" </span>",'<b class="color-orange run-0" title="Difference from fastest solution (with fastest results)">'+p(k=>k.runs[0].diff)+"</b>","|",'<span title="Number of cycles">'+p(k=>"x"+k.runs[0].count)+"</span>",...u.runs[0].executions.map((k,$)=>"<b>"+p(g=>{var w;return(w=g.runs[o].executions)==null?void 0:w[$]})+"</b>")].join(" ");else{u.$benchmark.querySelector(".log").innerHTML=['<span class="title">'+u.title.replace(/\s/g," ")+" </span>",u.runs.filter($=>$.executions).map(($,g)=>{const w=`n=${chunks[g]*getChunkLength()}`;return(`<b class="color-orange run-${g}" title="Difference from fastest solution (with fastest results)">`+p(v=>v.runs[g].diff)+' <span class="color-gray">'+p(v=>y(v.runs[g].count))+" <b>"+p(v=>v.runs[g].duration)+"</b></span></b>").padStart(w.length," ")}).join(" | ")].join("  ");const k=document.querySelector(".run-title");k.innerHTML=[" ".repeat(u.title.length)+" ",chunks.map(($,g)=>{const w=`n=${$*getChunkLength()}`;if(!u.runs[g].executions)return"";const S=p(v=>v.runs[g].diff)+" "+p(v=>y(v.runs[g].count))+" "+p(v=>v.runs[g].duration);return w.padStart((S.length+w.length)/2," ").padEnd(S.length," ")}).filter(Boolean).join(" | ")].join("  ")}}})))}}}function executeFunctionInWorker(e){return new Promise(t=>{const n=new Blob([`
      let start = performance.now();
      const result = (${e.toString()})();
      postMessage({result, duration: performance.now() - start});
      `],{type:"application/javascript"});new Worker(URL.createObjectURL(n)).addEventListener("message",c=>{t(c.data)})})}async function benchmarkScript(e,t=""){const n=document.createElement("script"),r=Math.random().toString().slice(2).split("").reverse().join("");return n.src="data:text/javascript;base64,"+btoa(`
        try {
            ${t}
            const __$start$ = performance.now();
            ${e};
    ...
    `),document.body.appendChild(n),new Promise((c,l)=>{n.onload=()=>{if(window["__error"+r]){const b=window["__error"+r];delete window["__error"+r],l(b)}const d=window["__duration"+r];delete window["__duration"+r],location.host==="localhost"||n.remove(),c(d)},n.onerror=l})}async function evalScript(e,t=""){const n=document.createElement("script"),r=Math.random().toString().slice(2).split("").reverse().join(""),c="`"+e.replaceAll("\\","\\\\").replaceAll("`","\\`")+"`";return n.src="data:text/javascript;base64,"+btoa(`
        try {
            ${t}
    ...
    `),document.body.appendChild(n),new Promise((l,d)=>{n.onload=()=>{if(window["__error"+r]){const o=window["__error"+r];delete window["__error"+r],d(o)}const b=window["__result"+r];delete window["__result"+r],location.host==="localhost"||n.remove(),l([b])},n.onerror=d})}function config(name,...vals){try{return eval(name)}catch(e){return vals.find(Boolean)}}})();
