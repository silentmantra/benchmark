...
`).map(o=>o.length));r=r.replaceAll("---","-".repeat(s));const l=document.querySelector("dialog"),d=l.querySelector("textarea");l.style.opacity=0,d.textContent=r,l.showModal(),d.focus(),d.select(),document.execCommand("copy"),l.close()});let running=!1;$container.querySelector(".stop-benchmark").addEventListener("click",()=>running=!1),$container.querySelector(".run-benchmark").addEventListener("click",async e=>{document.body.classList.add("benchmarking","running"),await Promise.timeout(),running=!0,await runBenchmark(),running=!1,document.body.classList.remove("running"),document.querySelector(".run-status").innerHTML=""});async function runBenchmark(){const e=[];groups.forEach(o=>{o.tests.forEach(c=>{c.$benchmark.querySelector(".log").textContent="",c.runs=Array.from(chunks,(i,h)=>e.tail={test:c,chunkCount:i,executions:[],diminisher:1,get duration(){return this.executions[0]??this._estimatedDuration},set duration(f){this._estimatedDuration=f},get count(){return o.count??o.runs[h].count/this.diminisher??1},get min(){return(this.executions[0]??this.duration)*this.diminisher}})})}),await Promise.requestAnimationFrame();let t=config("CYCLES",0);const n=+$container.querySelector("input.run-count").valueAsNumber||5,r=$container.querySelector(".run-multicore").checked,s=o=>document.querySelector(".run-status").innerHTML=o+'&nbsp;<span class="blink">...</span>',l=new Queue({depth:numberOfWorkers});t?groups.forEach(o=>o.count=t):(s("estimating cycles"),await Promise.all(groups.map(o=>Promise.all([...chunks.entries()].map(([c,i])=>l.push(async()=>{var _;let h=1,f=o.tests;const x=[];for(;;){for(const m of f)if(m.runs[c].duration=await m.benchmark(h,i),!running)return;const y=f.filter(m=>m.runs[c].duration<20);if(x.push(...f.filter(m=>m.runs[c].duration>=100)),x.forEach(m=>m.runs[c].diminisher*=10),!y.length){((_=o.runs??(o.runs=[]))[c]??(_[c]={})).count=h*10;break}f=f.filter(m=>m.runs[c].duration<100),h*=10}})))))),s("estimating results with workers (multi-threading)"),await Promise.all(e.map(o=>l.push(async()=>{o.duration=await o.test.benchmarkInWorker(o.count,o.chunkCount)}))),b();const d=new Queue({depth:r?numberOfWorkers:1,delay:r?0:500,running:()=>running});return Promise.all(Array.from({length:n},(o,c)=>Promise.all([...chunks.entries()].map(([i,h])=>Promise.all(groups.map(({tests:f,$benchmarks:x,runs:_})=>Promise.all(f.map(async y=>d.push(async()=>{var u;const m=y.runs[i];s(formatOrdinals(c+1)+" "+(chunks.length>1?`running ${y.title} with n=${h*getChunkLength()}`:`running ${y.title}`)),(u=y.$benchmark.querySelector(`.run-${i}`))==null||u.classList.add("blink"),await Promise.requestAnimationFrame();const a=await y[r?"benchmarkInWorker":"benchmark"](m.count,h);running&&(m.executions.push(a),m.executions.sort((p,k)=>p-k),b())})))))))));function b(){[...chunks.entries()].forEach(([o,c])=>groups.forEach(({tests:i,$benchmarks:h,runs:f})=>i.forEach(x=>{i.sort((a,u)=>a.runs[o].min-u.runs[o].min).forEach(a=>h.appendChild(a.$benchmark));const _=i[0].runs[o].min;i.forEach((a,u)=>{if(u){const p=a.runs[o].min/_;a.runs[o].diff=isNaN(p)?"":p.toFixed(2)+"x"}else a.runs[o].diff="1.00x"}),i.sort((a,u)=>(a=a.runs.findLast(p=>p.executions.length),u=u.runs.findLast(p=>p.executions.length),!a||!u?0:a.min-u.min)).forEach(a=>h.appendChild(a.$benchmark));const y=a=>{if(isNaN(a))return"";const u=a.toString().length,p=(u-1)/3|0,k=["","k","m","b"];return"x"+a.toString().slice(0,1+(u-1)%3)+k[p]},m=a=>(a/1e3).toFixed(1).replace(/^0/,"")+"s";for(const[a,u]of i.entries()){const p=k=>{const $=i.map(k);return($[a]??"").toString().padStart(Math.max(...$.map(w=>(w==null?void 0:w.toString().length)||0))," ")};if(chunks.length===1)u.$benchmark.querySelector(".log").innerHTML=['<span class="title">'+u.title.replace(/\s/g," ")+" </span>",'<b class="color-orange run-0" title="Difference from fastest solution (with fastest results)">'+p(k=>k.runs[0].diff)+"</b>","|",'<span title="Number of cycles">'+p(k=>"x"+k.runs[0].count)+"</span>",...u.runs[0].executions.map((k,$)=>"<b>"+p(g=>{var w;return(w=g.runs[o].executions)==null?void 0:w[$]})+"</b>")].join(" ");else{u.$benchmark.querySelector(".log").innerHTML=['<span class="title">'+u.title.replace(/\s/g," ")+" </span>",u.runs.filter($=>$.executions).map(($,g)=>{const w=`n=${chunks[g]*getChunkLength()}`;return(`<b class="color-orange run-${g}" title="Difference from fastest solution (with fastest results)">`+p(v=>v.runs[g].diff)+' <span class="color-gray">'+p(v=>y(v.runs[g].count))+" <b>"+p(v=>v.runs[g].duration)+"</b></span></b>").padStart(w.length," ")}).join(" | ")].join("  ");const k=document.querySelector(".run-title");k.innerHTML=[" ".repeat(u.title.length)+" ",chunks.map(($,g)=>{const w=`n=${$*getChunkLength()}`;if(!u.runs[g].executions)return"";const S=p(v=>v.runs[g].diff)+" "+p(v=>y(v.runs[g].count))+" "+p(v=>v.runs[g].duration);return w.padStart((S.length+w.length)/2," ").padEnd(S.length," ")}).filter(Boolean).join(" | ")].join("  ")}}})))}}}function executeFunctionInWorker(e){return new Promise(t=>{const n=new Blob([`
    ...
    `),document.body.appendChild(n),new Promise((s,l)=>{n.onload=()=>{if(window["__error"+r]){const b=window["__error"+r];delete window["__error"+r],l(b)}const d=window["__duration"+r];delete window["__duration"+r],location.host==="localhost"||n.remove(),s(d)},n.onerror=l})}async function evalScript(e,t=""){const n=document.createElement("script"),r=Math.random().toString().slice(2).split("").reverse().join(""),s="`"+e.replaceAll("\\","\\\\").replaceAll("`","\\`")+"`";return n.src="data:text/javascript;base64,"+btoa(`
        try {
            ${t}
