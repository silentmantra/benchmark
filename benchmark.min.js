...
`;HTMLElement.prototype.insert=function(n,s,...r){if(typeof r[0]=="object"){const t=r.shift();s=new Function(Object.keys(t).join(","),"return `"+s+"`")(...Object.values(t))}const o=document.createElement("div");o.innerHTML=s;const p=o.children[0];return this.insertAdjacentElement(n,o.children[0]),[p,...r.map(t=>p.querySelector(t))]},HTMLElement.prototype.append=function(){return this.insert("beforeend",...arguments)},HTMLElement.prototype.prepend=function(){return this.insert("afterbegin",...arguments)},Promise.timeout=function(n){return new Promise(s=>setTimeout(s,n))},Promise.requestAnimationFrame=function(){return new Promise(n=>requestAnimationFrame(n))},Array.prototype.trim=function(n=s=>!s){const s=(p,t)=>!n(p,t,this),r=this.findIndex(s);if(r===this.length-1)return[];const o=this.findLastIndex(s);return this.splice(o+1),this.splice(0,r),this};const containerTpl=`<div class="benchmarks">
    ...
    `;let beforeScript="";const scripts=[];let group="_default";const _groups={};let script,commented=!1;for(let n of scriptContainer.textContent.split(`
`)){if(/^\s*\/\*/.test(n)){commented=!0;continue}else if(commented&&/\*\/\s*$/.test(n)){commented=!1;continue}else if(commented)continue;if(/^\s*\/\/\s*@(end|footer)|^\s*\/\*\s*@end\s*\*\//.test(n))break;const s=n.match(new RegExp("(?<=@group\\s+).+(?=\\s*)$"));if(s){group=s[0];continue}const r=n.match(new RegExp("(?<=@benchmark\\s+).+(?=\\s*)$"));if(r){script&&scripts.push(script),_groups[group]=!0,script={title:r[0],group,source:""};continue}if(script){script.source+=n+`
`;continue}n.match(/^\s*const\s+\$input/)&&(n+="/* @input fill */"),beforeScript+=n+`
`}script&&scripts.push(script);{const n=Math.max(...scripts.map(s=>s.title.length));scripts.forEach(s=>s.title=s.title.padEnd(n," "))}const[$container,$groups]=document.body.append(eval("`"+containerTpl+"`"),".test-list"),groups=[];for(const n of Object.keys(_groups)){const[s,r]=$groups.append(groupTpl,{title:n},".benchmarks"),o={title:n,$group:s,$benchmarks:r,tests:[]};for(const p of scripts.filter(({group:t})=>t===n)){const[t,e]=r.append(benchmarkTpl,{...p},".eval");(async()=>{let h;try{const d=config("$chunk");let y=beforeScript;d&&(y=y.replace("/* @input fill */",inputFillScript()));const[l]=await evalScript(p.source,y+";");h=JSON.stringify(l,null,4)}catch(d){h=d.message,e.classList.add("error")}let[i,m]=p.source.split(/\/\/\s*@run/);m||([m,i]=[i,m]),i??(i=""),e.innerHTML=[p.title.replace(/\s/g,"&nbsp;")+"&nbsp:",h.toString().slice(0,500)].join("&nbsp;&nbsp;"),o.tests.push({...p,code:m,pre:i,$benchmark:t,benchmark(d=1,y=1){const l=config("$chunk");let g=beforeScript;return l&&(g=g.replace("/* @input fill */",inputFillScript(y))),g+=";"+i,benchmarkScript(d===1?m:`for (let $__i = 0; $__i < ${d}; $__i++) {${m}}`,g)}})})()}groups.push(o)}$container.querySelector(".cycles").value=config("CYCLES",scriptContainer.getAttribute("benchmark"),scriptContainer.dataset.count),$container.querySelector(".run-count").value=config("TIMES",5),$container.querySelector(".btn-copy").addEventListener("click",n=>{var h;const s=(h=navigator.userAgent.match(/(Chrome|Firefox)\/\d+/))==null?void 0:h[0];let r=document.querySelector(".run-title").textContent;r.length&&(r=r.replace(/^./,">"),r+=`
`);let o=`
        \`\`\`
        \` ${s}
        ---
...
`).map(i=>i.length));o=o.replaceAll("---","-".repeat(p));const t=document.querySelector("dialog"),e=t.querySelector("textarea");t.style.opacity=0,e.textContent=o,t.showModal(),e.focus(),e.select(),document.execCommand("copy"),t.close()});let running=!1;$container.querySelector(".stop-benchmark").addEventListener("click",()=>running=!1),$container.querySelector(".run-benchmark").addEventListener("click",async n=>{document.body.classList.add("benchmarking","running"),await Promise.timeout(),running=!0,await runBenchmark(),running=!1,document.body.classList.remove("running"),document.querySelector(".run-status").innerHTML=""});async function runBenchmark(){var o,p;groups.forEach(t=>{t.tests.forEach(e=>{e.$benchmark.querySelector(".log").textContent="",e.runs=Array.from(chunks,()=>({diminisher:1}))})}),await Promise.requestAnimationFrame();let n=$container.querySelector("input.cycles").valueAsNumber;const s=+$container.querySelector("input.run-count").valueAsNumber||5,r=t=>document.querySelector(".run-status").innerHTML=t+'&nbsp;<span class="blink">...</span>';if(n)groups.forEach(t=>t.count=n);else{r("estimating cycles");for(const t of groups)for(const[e,h]of chunks.entries()){let i=1,m=t.tests;const d=[];for(;;){for(const l of m)if(l.runs[e].duration=await l.benchmark(i,h),!running)return;const y=m.filter(l=>l.runs[e].duration<20);if(d.push(...m.filter(l=>l.runs[e].duration>=100)),d.forEach(l=>l.runs[e].diminisher*=10),!y.length){((o=t.runs??(t.runs=[]))[e]??(o[e]={})).count=i*10;break}m=m.filter(l=>l.runs[e].duration<100),i*=10}}}for(let t=0;t<s;t++)for(const[e,h]of chunks.entries()){for(const{tests:i,$benchmarks:m,runs:d}of groups)for(const[y,l]of[...i].entries()){const g=l.runs[e];r(formatOrdinals(t+1)+" "+(chunks.length>1?`running ${l.title} with n=${h*getChunkLength()}`:`running ${l.title}`)),(p=l.$benchmark.querySelector(`.run-${e}`))==null||p.classList.add("blink"),await Promise.requestAnimationFrame();const _=await l.benchmark(d[e].count/g.diminisher|0,h);if(!running)return;(g.executions??(g.executions=[])).push(_),g.executions.sort((c,a)=>c-a),g.min=g.executions[0],i.sort((c,a)=>c.runs[e].min*c.runs[e].diminisher-a.runs[e].min*a.runs[e].diminisher).forEach(c=>m.appendChild(c.$benchmark));const C=(i[0].runs[e].min||0)*i[0].runs[e].diminisher;i.forEach((c,a)=>{if(a){const u=c.runs[e].min*c.runs[e].diminisher/C;c.runs[e].diff=isNaN(u)?"":u.toFixed(2)+"x"}else c.runs[e].diff="1.00x"}),i.sort((c,a)=>(c=c.runs.findLast(u=>u.executions),a=a.runs.findLast(u=>u.executions),!c||!a?0:c.min*c.diminisher-a.min*a.diminisher)).forEach(c=>m.appendChild(c.$benchmark));const S=c=>{if(isNaN(c))return"";const a=c.toString().length,u=(a-1)/3|0,v=["","k","m","b"];return"x"+c.toString().slice(0,1+(a-1)%3)+v[u]},L=c=>(c/1e3).toFixed(1).replace(/^0/,"")+"s";for(const[c,a]of i.filter(u=>u.runs[e].executions).entries()){const u=v=>{const w=i.map(v);return(w[c]??"").toString().padStart(Math.max(...w.map(f=>(f==null?void 0:f.toString().length)||0))," ")};if(chunks.length===1)a.$benchmark.querySelector(".log").innerHTML=['<span class="title">'+a.title.replace(/\s/g," ")+" </span>",'<b class="color-orange run-0" title="Difference from fastest solution (with fastest results)">'+u(v=>v.runs[0].diff)+"</b>","|",'<span title="Number of cycles">'+u(v=>"x"+d[0].count/v.runs[0].diminisher)+"</span>",...a.runs[0].executions.map((v,w)=>"<b>"+u(b=>{var f;return(f=b.runs[e].executions)==null?void 0:f[w]})+"</b>")].join(" ");else{a.$benchmark.querySelector(".log").innerHTML=['<span class="title">'+a.title.replace(/\s/g," ")+" </span>",a.runs.filter(w=>w.executions).map((w,b)=>{const f=`n=${chunks[b]*getChunkLength()}`;return(`<b class="color-orange run-${b}" title="Difference from fastest solution (with fastest results)">`+u(k=>k.runs[b].diff)+' <span class="color-gray">'+u(k=>S(d[b].count/k.runs[b].diminisher))+" <b>"+u(k=>{var x;return(x=k.runs[b].executions)==null?void 0:x[0]})+"</b></span></b>").padStart(f.length," ")}).join(" | ")].join("  ");const v=document.querySelector(".run-title");v.innerHTML=[" ".repeat(a.title.length)+" ",chunks.map((w,b)=>{const f=`n=${w*getChunkLength()}`;if(!a.runs[b].executions)return"";const $=u(k=>k.runs[b].diff)+" "+u(k=>S(d[b].count/k.runs[b].diminisher))+" "+u(k=>{var x;return(x=k.runs[b].executions)==null?void 0:x[0]});return f.padStart(($.length+f.length)/2," ").padEnd($.length," ")}).filter(Boolean).join(" | ")].join("  ")}}if(l!==i.at(-1)&&await Promise.timeout(1e3),!running)return}if(t<s.length-1&&await Promise.timeout(1e3),!running)return}}}async function benchmarkScript(n,s=""){const r=document.createElement("script"),o=Math.random().toString().slice(2).split("").reverse().join("");return r.src="data:text/javascript;base64,"+btoa(`
        try {
            ${s}
    ...
    `),document.body.appendChild(r),new Promise((p,t)=>{r.onload=()=>{if(window["__error"+o]){const h=window["__error"+o];delete window["__error"+o],t(h)}const e=window["__duration"+o];delete window["__duration"+o],r.remove(),p(e)},r.onerror=t})}async function evalScript(n,s=""){const r=document.createElement("script"),o=Math.random().toString().slice(2).split("").reverse().join(""),p="`"+n.replaceAll("\\","\\\\").replaceAll("`","\\`")+"`";return r.src="data:text/javascript;base64,"+btoa(`
        try {
            ${s}
    ...
    `),document.body.appendChild(r),new Promise((t,e)=>{r.onload=()=>{if(window["__error"+o]){const i=window["__error"+o];delete window["__error"+o],e(i)}const h=window["__result"+o];delete window["__result"+o],r.remove(),t([h])},r.onerror=e})}function config(name,...vals){try{return eval(name)}catch(n){return vals.find(Boolean)}}})();
